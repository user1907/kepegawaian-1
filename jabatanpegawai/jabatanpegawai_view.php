...
$idjp = $_GET['idjp'];
$stmt = mysqli_prepare($koneksi, "SELECT jp.*, p.namalengkap, j.namajabatan FROM jabatanpegawai jp JOIN pegawai p ON jp.nip = p.nip JOIN jabatan j ON jp.kodejabatan = j.kodejabatan WHERE jp.idjp=?");
...
<body>
    ...
    <div class="container">
        ...
        <table>
            <tr><td>ID</td><td><?= htmlspecialchars($row['idjp']) ?></td></tr>
            <tr><td>NIP</td><td><?= htmlspecialchars($row['nip']) ?> - <?= htmlspecialchars($row['namalengkap']) ?></td></tr>
            <tr><td>Kode Jabatan</td><td><?= htmlspecialchars($row['kodejabatan']) ?> - <?= htmlspecialchars($row['namajabatan']) ?></td></tr>
            <tr><td>Status</td><td><?= htmlspecialchars($row['status']) ?></td></tr>
            <tr><td>Periode</td><td><?= htmlspecialchars($row['periode']) ?></td></tr>
        </table>
        <a href="jabatanpegawai_list.php" class="btn">Kembali</a>
    </div>
</body>
</html>
